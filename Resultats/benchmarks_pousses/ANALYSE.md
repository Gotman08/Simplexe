...
## Comparaison avec le benchmark précédent

> Ce benchmark est le **second run complet**, après les modifications C
> destinées à éliminer les warnings (`-Wconversion`, `-Wstrict-overflow`,
> rewrite des bornes de boucle `i < n - 1` → `i + 1 < n`).
> Le précédent CSV est conservé dans `Resultats/benchmarks_pousses_avant/`
> pour comparaison reproductible (`scripts/_compare_benchmarks.sh`).

### Résultats clés de la comparaison

| Aspect | Constat |
|---|---|
| **Correctness** | ✅ Solutions identiques sur **toutes** les instances (UKP et TSP). 0 régression. |
| **OR-Tools UKP** | **Plus rapide** sur l'ensemble : −10 % à −43 % (probablement effet warmup CBC). |
| **OR-Tools TSP** | Stable (timeout dominant) sauf usa_13509 +3.7 % (variance run-to-run). |
| **BB (C) UKP < 1 ms** | +20 à +78 % apparent — bruit en microsecondes, négligeable en absolu. |
| **BB (C) UKP ≥ 10 ms** | Stable (−2.5 % à +0.5 %). |
| **BB (C) TSP petits** | Légèrement plus rapide (−2 à −6 %). |
| **BB (C) TSP moyens** | pd_20 +8.4 % et pd_22 +12.9 % — explication ci-dessous. |
| **Audit qualité** | 7 OK + 4 WARN (anomalies CBC connues) + **0 ERROR**. |

### Analyse de la régression BB sur TSP moyens

La rewrite des bornes `for (i = 0; i < n - 1; ++i)` → `for (i = 0; i + 1 < n; ++i)`
empêche gcc d'appliquer une transformation `cmp` qu'il faisait avant
(`-Wstrict-overflow=5` les détectait, donc les corriger les a aussi désactivées).

**Trade-off** : un peu de performance perdue (~10 % sur les BB longs) en
échange d'un code certifié 0 warning sous tous les flags stricts.

L'utilisateur a explicitement demandé du code sans warning, donc cette
légère perte est attendue et acceptable. Pour récupérer la performance
sans warning, il faudrait soit :
- ajouter `-Wno-strict-overflow` (non souhaité — masque le warning),
- réécrire les boucles avec `size_t` partout (refactor profond).

Tous les autres aspects sont stables ou améliorés.
